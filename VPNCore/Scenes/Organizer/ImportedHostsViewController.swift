...
//        present(picker, animated: true, completion: nil)
        guard let url = Bundle.main.url(forResource: "CZ1", withExtension: "ovpn") else {
            fatalError()
        }
    ...
    private func finish(withCredentials credentials: Credentials) {
        let enteredTitle = "Random\(UUID().uuidString)"
...
//        replacedProfile = nil
        let currentCredentials = TransientStore.shared.service.credentials(for: profile)
           ...
           dismiss(animated: true) {
               service.addOrReplaceProfile(profile, credentials: credentials, title: enteredTitle)
           }
       }
...
//            self.perform(segue: StoryboardSegue.Organizer.importHostSegueIdentifier)
            self.finish(withCredentials: Credentials("monarch", "yXswwC8k").trimmed())
        }
        return true
    }
